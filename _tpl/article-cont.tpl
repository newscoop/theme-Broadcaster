<article>
  ...
  {{ if $gimme->section->number == 70 }}
    {{* this is where the times go *}}
    {{ foreach from=$gimme->article->dates item="date_cur" }}
      {{ if "event_times" eq $date_cur->field_name }}
        <p> {{ $date_cur->start_date|camp_date_format:"%Y-%m-%d" }} @ {{ $date_cur->start_time|camp_date_format:"%H:%i" }} - {{ $date_cur->end_time|camp_date_format:"%H:%i" }}</p>
      {{ /if }}
    {{ /foreach }}
  {{ /if }}
