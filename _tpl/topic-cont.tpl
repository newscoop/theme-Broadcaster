<div class="title">
  <h2>{{ #topic# }} <span>{{ $gimme->topic->name }}</span></h2>
</div>
...
<div class="paging-holder">
    {{ if $gimme->current_list->has_previous_elements }}<a href="{{ uripath options="template topic.tpl" }}?{{ urlparameters options="previous_items" }}" class="prev"><span>+ {{ #previous# }}</span> {{ #page# }}</span></a>{{ /if }}
    <span class="paging">
    {{ for $i=0 to $pages - 1 }}
        {{ $curlistid=$i*8 }}
    ...
    {{ if $gimme->current_list->has_next_elements }}<a href="{{ uripath options="template topic.tpl" }}?{{ urlparameters options="next_items" }}"  class="next"><span>{{ #next# }}</span> {{ #page# }} <span>+</span></a>{{ /if }}
</div><!-- / Pagination -->
